...
# 海报图片
pic_res = re.compile(r'<div id="mainpic" class="">.*? <img src="(.*?)".*? />', re.S)
...
# 制片国家/地区
obj = re.compile(r'<span class="pl">制片国家/地区:</span> (.*?)<br/>', re.S)  # re.S让'.'匹配换行符
result = obj.findall(page_text)
print('◎制片国家/地区：',''.join(result))
md.write('◎制片国家/地区：' + ''.join(result) + '<br>')
# 语言
language = re.findall(r'<span class="pl">语言:</span> (.*?)<br/>',page_text)
...
print('◎首播：', movie_date)
md.write('◎首播：' + ''.join( movie_date) + '<br>')
# 集数
numbers_re = re.compile(r'<span class="pl">集数:</span> (.*?)<br/>', re.S)
number = numbers_re.findall(page_text)
print('◎集数：', number[0])
md.write('◎集数：' + ''.join(number) + '<br>')
# 单集片长
time_re = re.compile(r'<span class="pl">单集片长:</span> (.*?)<br/>', re.S)
time = time_re.findall(page_text)
print('◎单集片长：', time[0])
md.write('◎单集片长：' + ''.join(time) + '<br>')
# imdb代码
imdb_re = re.compile(r'<span class="pl">IMDb:</span> (.*?)<br>', re.S)
imdb = imdb_re.findall(page_text)
print('◎IMDb: ',''.join(imdb))
md.write('◎IMDb: ' + ''.join(imdb)+ '<br>')
# 简介
sum_re = re.compile(r'<span property="v:summary" class="">(.*?)</span>',re.S)
